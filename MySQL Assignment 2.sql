...
SELECT * 
FROM employees WHERE YEAR(hire_date) = 2018 ORDER BY hire_date ASC
LIMIT 5;

-- Aggregate Functions

SELECT SUM(salary) AS Finance_totalsalary FROM employees WHERE department_id = 7;
SELECT MIN(age) AS lowestage FROM employees;

 -- GROUP BY

SELECT location_id, MAX(salary) AS Higestsalary FROM employees GROUP BY location_id;
SELECT designation, AVG(salary) AS averagesalary FROM employees WHERE designation LIKE '%Analyst%'
...
HAVING COUNT(*) < 3;
SELECT location_id, AVG(age) AS average_age FROM employees WHERE gender = 'F'
